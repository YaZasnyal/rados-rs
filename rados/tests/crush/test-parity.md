...
| 4 | Choose arguments: positional weights/IDs and legacy encoding fallback | Passing; pinned C vectors cover 2,100 ordered mappings across direct FIRSTN/INDEP, recursive CHOOSELEAF, chained steps, and Tentacle MSR FIRSTN/INDEP with unavailable-device retries. They distinguish selected, default fallback, missing-without-default, and present-empty sets; `pg_to_osds` uses the pool index. The two unchanged QA map states and two source-shaped QA final states decode with their published canonical/compat totals. |
