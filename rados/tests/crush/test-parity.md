...
| 4 | Choose arguments: positional weights/IDs and legacy encoding fallback | Passing; 2,100 pinned C vectors cover direct FIRSTN/INDEP, recursive CHOOSELEAF, chained steps, and Tentacle MSR FIRSTN/INDEP with unavailable-device retries. They distinguish selected, default fallback, missing-without-default, and present-empty sets; `pg_to_osds` uses the pool index. All 16 published QA transition states have decoded canonical/compat assertions and 160 ordered C/Rust placement rows. CLI indexes 1–6 retain all declared bucket positions, weights and IDs; malformed source-byte mutations assert the bucket/weight/ID guards. |
